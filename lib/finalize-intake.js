...
// ---- Load & compile JSON Schema (OPTIONAL) ----
...
try {
  if (fs.existsSync(schemaPath)) {
    ...
    console.warn('[finalize-intake] No schema file found at', schemaPath, '- proceeding without validation.');
  }
} catch (e) {
  console.warn('[finalize-intake] Failed to read/parse schema. Proceeding without validation. Reason:', e?.message || e);
  schema = null;
}
...
if (schema) {
  try {
    ...
    console.warn('[finalize-intake] Failed to compile schema. Proceeding without validation. Reason:', e?.message || e);
    ajv = null;
    validate = null;
  }
}
...
function buildLLMPrompt(transcript, seed, schemaObj) {
  const schemaSection = schemaObj
    ? `JSON Schema (reference):\n${JSON.stringify(schemaObj, null, 2)}\n\n`
    : `No JSON Schema available. Still return a strict JSON object with the requested keys.\n\n`;
  ...
  return [
    {
      ...
      content:
        ...
        `- Follow the provided JSON Schema if present; otherwise use the requested keys only.`
    },
    {
      role: 'user',
      content:
`${schemaSection}Seed data from realtime extractors (may be partial or noisy):
...
async function callOpenAIForIntake(transcripts, seed, schemaObj) {
  const messages = buildLLMPrompt(transcripts, seed, schemaObj);
...
async function finalizeIntake(shadowState, meta = {}) {
  ...
  // Validate (if schema was present)
  const { ok, errors } = validateOrReport(merged);
  if (!ok) {
    console.warn('[finalize-intake] schema errors:', errors);
  }
  return merged;
}
